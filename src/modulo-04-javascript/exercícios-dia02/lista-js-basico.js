...
Number.prototype.converteParaKilos = function() {
...
var obterCavaleiroComMaisGolpes = function(){
  ...
  for(var i=0; i<goldSaints.length; i++){
    if(goldSaints[i].golpes.length > aux) {
      cavaleiroComMaisGolpes = goldSaints[i];
      aux = goldSaints[i].golpes.length;
    }
  }
...
var obterAlturaMedia = function(){
  ...
  media = (soma / goldSaints.length);
  return media.converterParaMetros().duasCasas();
}
...
var obterAlturaMediana = function(){
  ...
  mediana = (arrayOrd[arrayOrd.length / 2 -1].alturaCm + arrayOrd[arrayOrd.length / 2].alturaCm) / 2;
  return mediana.converterParaMetros().duasCasas();
}
...
var obterIMC = function(){
  ...
  for (var i = 0; i < goldSaints.length; i++) {
    for (var prop in goldSaints[i]) {
      if(prop === 'pesoLb'){
          imc = goldSaints[i].pesoLb.converteParaKilos() / (goldSaints[i].alturaCm.converterParaMetros() * goldSaints[i].alturaCm.converterParaMetros());
         array.push(imc.duasCasas());
      }
    }
  }
  return array;
